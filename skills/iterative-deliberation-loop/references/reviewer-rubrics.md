...
如果本轮 `AttemptResult.attempt_kind = exploratory_snapshot` 或 `completion_status = degraded`，默认降低 `structural_progress` 的证据权重。
...
- 如果 evidence gap 会改变下一控制决策，明确要求先过 `VerificationGate`
- `AttemptResult` 是主要审稿对象，不要回退成审 transcript
