...
`frame campaign -> plan attempt -> build AttemptPacket -> run discussion -> review AttemptResult -> decide next action -> update ledger -> repeat or stop`
...
### 3. `build_attempt_packet`

把 outer state 压缩成 boundary object，而不是把 controller 私有字段直接塞进 inner。

最小 `AttemptPacket` 应包含：
...
inner 只允许消费 `AttemptPacket`，并只允许返回 `AttemptResult`。

### 5. `review_attempt_result`

reviewer cell 审读 `AttemptResult`，而不是接管整个求解过程。
...
- 本次 attempt 真正新增了什么
- 哪些 objection 被引入、升级、合并，或状态改写为 `addressed / mitigated / closed / accepted_risk / reopened / escalated`
- 哪些 branch 还活着
- 下一轮要改什么

不要把 transcript 当 ledger。
不要再额外维护 `Attempt Resume Packet`。
